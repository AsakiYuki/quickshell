...
const LUCKY_HOURS = ["110100101100", "001101001011", "110011010010", "101100110100", "001011001101", "010010110011"]
const SOLAR_TERMS = [
	"Xuân phân",
	"Thanh minh",
	"Cốc vũ",
...
class LunarDate extends Calendar {
	...
	static getRecommended(date) {
		...
		date.jd = rcm.jd
		date.leap_month = rcm.leap_month
		date.leap_year = rcm.leap_year
		date.length = rcm.length
		return d
	}
	...
	static decodeLunarYear(year, year_code) {
		...
		for (let month = 1; month <= 12; month++) {
			...
			const d = date
			d.leap_month = false
			let normal_lunar = new LunarDate(d)
			...
			if (leapMonth === month) {
				const d = date
				d.leap_month = true
				let leap_lunar = new LunarDate(d)
	...
	get() {
		const g = super.get()
		g.year_name = this.getYearName()
		g.leap_month = this.leap_month
		return g
	}
}
