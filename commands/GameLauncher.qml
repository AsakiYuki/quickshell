        ...
        const manifestsPromise = fs.readfile(`${Paths.steam}/steamapps/libraryfolders.vdf`).then(vdf => {
            ...
            for (const { apps, path } of folders)
                for (const id in apps)
                    reads.push(fs.readfile(`${path}/steamapps/appmanifest_${id}.acf`))
            return Promise.all(reads).then(contents =>
                contents.map(c => VdfParser.parse(c).AppState)
            )
        })
        ...
        Promise.all([iconsPromise, manifestsPromise]).then(([icons, apps]) => {
            ...
            for (const v of apps) {
                ...
                entries.push({
                    ...
                    execute: () => chillProcess.exec(["steam", `steam://rungameid/${v.appid}`])
                })
            }
